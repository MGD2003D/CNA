...
5. Деплоим. Проверяем в логах, что зукипер запустился, и что одна нода постгреса из двух стала лидером/овнером/мастером.
   ...
   *При обычном перезапуске композ-проекта, будет ли сбилден заново образ? А если предварительно отредактировать файлы postgresX.yml? А если содержимое самого Dockerfile? Почему?*
   ...
   При обычном перезапуске композ-проекта образ не будет сбилден заново. Будут использоваться уже существующие образы, если они были созданы ранее.
   ...
   Редактирование Dockerfile не приведет к пересборке образа автоматически при перезапуске композ-проекта. Изменения в Dockerfile не отслеживаются docker compose. Используется уже существующий образ, если он был создан ранее. Чтобы применить изменения, необходимо вручную пересобрать образ.
